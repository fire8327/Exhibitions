        ...
        <FormKit @submit="authUser" type="form" :actions="false" messages-class="hidden"
            ...
            <div class="flex items-center lg:items-start gap-4 flex-col w-full md:w-2/3 lg:w-1/2">
                <FormKit v-model="user.login" validation="required" messages-class="text-[#E9556D] font-mono"
                    ...
                    input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-cyan-500 shadow-md" />
            ...
            <button type="submit" :disabled="isAuthDisabled"
                ...
                class="px-4 py-2 border border-cyan-500 bg-cyan-500 text-white rounded-full w-[160px] text-center transition-all duration-500 hover:text-cyan-500 hover:bg-transparent">
                Вход
            </button>
        </FormKit>
        <div
            ...
            <p class="mainHeading max-w-md">
                Еще нет аккаунта? <span class="text-cyan-500">Зарегистрируйтесь</span>
            </p>
            <NuxtLink to="/reg"
